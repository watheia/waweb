import { EnterOperandCommand, Operand } from "../../../../src/app"
...
export const EnterOperand = (operand: Operand) =>
  Interaction.where(`#actor enters an operand of ${operand.value}`, (actor: Actor) => {
    const ability = InteractDirectly.as(actor)

    const command = new EnterOperandCommand(operand, ability.currentCalculationId())
