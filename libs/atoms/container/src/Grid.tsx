...
function Grid(props: GridProps, ref: DOMRef<HTMLDivElement>) {
  ...
  const { styleProps } = useStyleProps(otherProps, gridStyleProps)
...
function gridTemplateAreasValue(value: any) {
  return value.map((v: any) => `"${v}"`).join("\n")
}

function gridDimensionValue(value: any) {
...
function gridTemplateValue(value: any) {
